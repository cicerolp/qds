...
// TODO pass std::array
void Pivot::merge_pdigest(const Pivot &pivot) {
  add(std::vector<float>(pivot._mean.begin(), pivot._mean.end()),
      std::vector<float>(pivot._weight.begin(), pivot._weight.end()));
}
