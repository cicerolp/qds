...
static int left(const struct frozen *f) { return f->end - f->cur; }
...
static void mg_ev_mgr_free(struct mg_mgr *mgr) { (void)mgr; }

static void mg_ev_mgr_add_conn(struct mg_connection *nc) { (void)nc; }

static void mg_ev_mgr_remove_conn(struct mg_connection *nc) { (void)nc; }
...
static int deliver_websocket_data(struct mg_connection *nc) {
  ...
  int ok,
      reass = buf_len > 0 && is_ws_fragment(p[0]) &&
...
static unsigned char *mg_parse_dmg_resource_record(
  ...
  if (reply) {
    ...
    rr->ttl = (uint32_t)data[0] << 24 | (uint32_t)data[1] << 16 | data[2] << 8 |
              data[3];
    data += 4;
  ...
  if ((err = RegOpenKey(HKEY_LOCAL_MACHINE, key, &hKey)) != ERROR_SUCCESS) {
    ...
    for (ret = -1, i = 0;
      ...
      if (RegOpenKey(hKey, subkey, &hSub) == ERROR_SUCCESS &&
          (RegQueryValueEx(hSub, "NameServer", 0, &type, (void *)value, &len) ==
               ERROR_SUCCESS ||
