...
class BinnedPivot {
 public:
  ...
  BinnedPivot(const Pivot& pivot) : pivot(pivot) {}
  ...
  static inline bool is_sequence(const BinnedPivot& lhs,
                                 const BinnedPivot& rhs) {
    return Pivot::is_sequence(lhs.pivot, rhs.pivot) && lhs.value == rhs.value;
  }

  static inline bool upper_bound_comp(const BinnedPivot& lhs,
                                      const Pivot& rhs) {
