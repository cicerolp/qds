	...
	for (const auto& schema : args) {
      start = std::chrono::high_resolution_clock::now();
	   ...
	   if (telemetry) {
         ...
         (*NDSInstances::getInstance()._telemetry_files[schema.name]) << "memory,time2build" << std::endl;

         auto duration = std::chrono::duration_cast<std::chrono::seconds>(end - start).count();
         auto memory = getCurrentRSS() / (1024 * 1024);

         (*NDSInstances::getInstance()._telemetry_files[schema.name]) << memory << ","  << duration << std::endl;
	   }
	}
}
