...
data <- rbind(
  ...
  load_db('log_20190107-192939/db_20190107-192939_twitter-small-count-region.csv', 'twitter-small')
)
```
...
```{r fig.height=2.6, fig.width=5.7}
# {r fig.height=2.6, fig.width=6}
...
data %>% 
  ...
  ggplot(aes(x = Structure, y = Mean)) + 
    ...
    theme(
      ...
      # legend.title = element_text(size = 11),
      # legend.title = element_blank(),
      legend.margin = margin(t = -0.1, b = -0.2, r = -0.4, unit='cm'),
      plot.margin = unit(c(0,0.0,0,-0.9), "lines"),
      axis.text.y = element_blank(),
      axis.ticks = element_blank(),
      strip.text.y = element_text(size = 8)
    ) +
