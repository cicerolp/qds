...
```{r}
db_brightkite <- read_delim('db_20181207-144925_brightkite-count-region.csv', delim = ';', col_names = FALSE)

db_brightkite <- db_brightkite %>%
  group_by(X3, X5) %>%
  summarise(
    Time = sum(X7),
    Structure = X8
...
unique(db_brightkite$Structure)
```
...
data %>% 
  ...
  ggplot(aes(x = Structure, y = Mean)) + 
    ...
    guides(colour = guide_legend(title = "Counting Queries"))
