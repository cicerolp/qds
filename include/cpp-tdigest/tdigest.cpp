  ...
  while (true) {
    ...
    if (nextIndex >= index) {
      if (previousMean == NAN) {
        ...
        previousMean =
            (nextIndex2 * _centroids->value(next) - nextIndex * _centroids->value(next2)) / (nextIndex2 - nextIndex);
      }
      ...
      const double nextMean2 =
          (_centroids->value(next) * (nextIndex2 - previousIndex) - previousMean * (nextIndex2 - nextIndex))
